...
public unsafe class ImGuiController : IDisposable
{
    ...
    public ImGuiController(int width, int height)
    {
        ...
        Shader = new Shader(
            ("OpenGL.Resources.Shaders.imgui.vert", ShaderType.VertexShader),
            ("OpenGL.Resources.Shaders.imgui.frag", ShaderType.FragmentShader));
    ...
    private void CreateContext()
    {
        ...
        SetupClipboard();
        var io = ImGui.GetIO();
        Stream? stream = ResourcesUtils.GetResourceStream("OpenGL.Resources.Fonts.FiraCode-Medium.ttf");
        IntPtr fontData = IntPtr.Zero;
        if (stream != null)
        {
            fontData = StreamToIntPtr(stream, out int dataLength);
            var ptr = io.Fonts.AddFontFromMemoryTTF(fontData, dataLength, 18.0f);
            ptr.ConfigData.FontDataOwnedByAtlas = false;
        }
        else
        {
