...
public class Texture : IDisposable, IBindable
{
    ...
    public Texture(Array data, int width, int height)
    {
        GL.CreateTextures(TextureTarget.Texture2D, 1, out int handle);
        Handle = handle;
        Allocate(width, height, SizedInternalFormat.Rgb8);
        Update(data, 0, 0, width, height, PixelFormat.Rgb, PixelType.UnsignedByte);
    ...
    {
        if (levels < 0) levels = BitOperations.Log2((uint)Math.Max(width, height)) + 1;
