...
0. Lecture / Laboratory Schedule, Subject Rules and Organization:
    ...
    * Laboratories, mgr inż. Cezary Bella: https://pages.mini.pw.edu.pl/~bellac/
    * Rules: https://usosweb.usos.pw.edu.pl/
...
2. MSBuild Workshop:
    ...
    * Add ```AssemplyName``` and ```OutputPath``` properties to customize the process. Remember about ```PropertyGroup```:
        ```xml
        <PropertyGroup>
    ...
    * TEACHERS: Give student 5 minutes to experiment, then show the solution and verify.
        ```bash
        msbuild HelloWorld.myproj -p:AssemblyName=MyNewAssemblyName
        ```
        Because you did not use the -t switch to explicitly set the target, MSBuild runs the default Build target. The -p switch overrides the AssemblyName property and gives it the new value, *MyNewAssemblyName*. This causes a new application, *MyNewAssemblyName.exe*, to be created in the \Bin\ folder.

        You can also verify that \Bin\ folder containg both *MSBuildHelloWorld.exe* and *MyNewAssemblyName.exe*.
        ```bash
        msbuild HelloWorld.myproj -t:Clean
        ```
        This deletes the default *AssemblyName* application (*MSBuildHelloWorld.exe*). To delete *MyNewAssemblyName.exe* you need to override the property once again:
        ```bash
        msbuild HelloWorld.myproj -t:Clean -p:AssemblyName=MyNewAssemblyName
        ```
        This runs the Clean task to remove the application that has the given *AssemblyName* property value, *MyNewAssemblyName*.

        ```bash
        msbuild HelloWorld.myproj -t:Rebuild
        ```
    ...
    * TEACHERS: Please do the walkthrough of below points with students (up to 'Output Type'), then let them experiment for 5 minutes and show the whole thing yourself.
    ...
    * Take a look at what was created in project file once you've modifies *Pre-Build Events* using Visual Studio.
    ...
    * TEACHERS: Show them how it's done by explaining default targets from SDK:
        ```xml
        ...
        ```
        Focus on ItemGroup/Compile from first entry and ItemGroup/None (first one) from second entry.
    ...
    * TEACHERS: Show students solution of using zip command:
        ```xml
        <Target Name="PostBuild" AfterTargets="PostBuildEvent">
            <ZipDirectory SourceDirectory="$(OutDir)" DestinationFile="$(ProjectDir)\Output.zip" />
        </Target>
        ```

    * TEACHERS: You can show them more advanced example of how to use this feature with conditions and wildcards:
        ```xml
        <ItemGroup>
            <Shader Include="**\*.vert" />
            <Shader Include="**\*.frag" />
        ...
        <Target Name="CompileShaders" BeforeTargets="Build">
            <Message Importance="high" Text="Compiling Spir-V Shaders"/>
            <Exec Command="@(Shader->'glslc %(Identity) -o %(Identity).spv')" />
            <Copy SourceFiles="@(Shader->'%(Identity).spv')" DestinationFiles="@(Shader->'$(OutDir)%(Identity).spv')"/>
            <ItemGroup>
                <EmbeddedResource Include="**/*.spv" />
            </ItemGroup>
        </Target>
        ```
        More info about items metadata on the [dedicated page](https://learn.microsoft.com/en-us/visualstudio/msbuild/msbuild-well-known-item-metadata?view=vs-2022) in the documentation.
...
4. .NET CLI Workshop:
    ...
    * TEACHERS: Show students how that looks like in Visual Studio.
    ...
    * TEACHERS: Show students how to add project reference in Visual Studio (right click on lab01 project -> add -> project reference -> check myClassLibrary project -> ok).
