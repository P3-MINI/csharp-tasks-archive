...
void ShowFilesystem(std::string offsetPrefix, std::filesystem::path currentPath)
{
    ...
    {
        std::cout << offsetPrefix << " - " << currentEntry.path() << std::endl;

        if (currentEntry.is_directory())
            ShowFilesystem(offsetPrefix + '\t', currentEntry.path());
    }
}
    ...
    */
    ...
    std::cout << " --------------------- ETAP_1 (2.0 Pts) ---------------------" << std::endl;
    {
        ...
        */
        ...
        {
			ShowFilesystem(std::string(), std::filesystem::current_path());
            ...
            for (auto& currentEntry : std::filesystem::directory_iterator(std::filesystem::current_path()))
            {
                auto lastWriteTime = std::filesystem::last_write_time(currentEntry.path());

                lastWriteTimes.push_back(lastWriteTime);

                if (currentEntry.is_directory() == false && distribution(randomEngine) < 0.3f)
                    std::filesystem::last_write_time(currentEntry.path(), lastWriteTime + 5s);
            }
            ...
            for (auto& currentEntry : std::filesystem::directory_iterator(std::filesystem::current_path()))
            {
                if (std::filesystem::last_write_time(currentEntry.path()) != lastWriteTimes[index++])
                    std::cout << "Changes To: " << currentEntry.path().filename() << std::endl;
            }
        }
    }
        ...
        */
...

        {
            ...
            for (int i = 0; i < threadsCount; i++)
            {
                ...
                futures.push_back(std::async(std::launch::async, [&vectorMutex, &threadIDs, amountLoops, displayValue]()
                {
                    std::cout << "Starting Thread No: " << std::this_thread::get_id() << std::endl;

                    /* Sleeps for 1s every loop and displays the value - emulates some work */
                    {
                        for (int k = 0; k < amountLoops; k++)
                        {
                            std::this_thread::sleep_for(1s);

                            std::cout << displayValue << std::flush;
                        }
                    }

                    /* Thread Safe push into vector using std::lock_guard that frees the mutex when goes out of scope */
                    {
                        std::lock_guard<std::mutex> currentLock(vectorMutex);
                        threadIDs.push_back(std::this_thread::get_id());
                    }

                    std::cout << std::endl << "Finishing Thread No: " << std::this_thread::get_id() << std::endl;
                }));
            }
        }

        {
            ...
            std::for_each(threadIDs.begin(), threadIDs.end(), [](const std::thread::id& currentID) { std::cout << currentID << ' '; });
        ...
        */

        {
            ...
            for (int i = 0; i < assets.size(); i++)
                assetManager.AddAsset(std::move(assets[i]));
