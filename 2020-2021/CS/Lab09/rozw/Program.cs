...
namespace Lab09_EN
{
    ...
    static class CircularBufferExtender
    {
        public static CircularBuffer<T> Clone<T>(this CircularBuffer<T> circularBuffer)
            where T : IComparable<T>
        {
            CircularBuffer<T> newCircularBuffer = new CircularBuffer<T>(circularBuffer.Size);
            foreach (T value in circularBuffer)
            {
                newCircularBuffer.Put(value);
            }
