    ...
    class Program
    {
        static void Main(string[] args)
        {
...
#if STAGE3
            {
                ...
                //f = |x-5|
                var f = new Func<double, double>(x => Math.Abs(x - 5));
                //f = sin(x)*1/10*x
                var g = new Func<double, double>(x => Math.Sin(x) * 0.1 * x);
...
#if STAGE4
            {
                ...
                //Uzupełnij
                var replacedDoubles = doubles.ReplaceIf(x => { return x - (int) x > 0.5; },
                    x => { return x / 2 - (x - (int) x); });
