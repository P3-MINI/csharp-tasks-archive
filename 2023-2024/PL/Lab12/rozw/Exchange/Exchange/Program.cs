...
class Program
{
    static void Main(string[] args)
    {
        ...
        {
            messages.Add($"Worker {i}, words count: {line.Split(" ").Length}");
        });
        ...
        Parallel.For(2, 1000, i =>
        {
            ...
            if (isPrime)
            {
                Interlocked.Increment(ref primesCount);
                Console.WriteLine(i);
            }
        });
...
#endif
#if STAGE_3
        var workerClientA = new BackgroundWorker();
        workerClientA.DoWork += (_, _) =>
        {
            Thread.CurrentThread.Name = "WorkerA";
            clientA.Run();
        };
        workerClientA.RunWorkerAsync();
        var workerClientB = new BackgroundWorker();
        workerClientB.DoWork += (_, _) =>
        {
            Thread.CurrentThread.Name = "WorkerB";
            clientB.Run();
        };
