...
public class Program : GameWindow
{
    ...
    private void RenderGui()
    {
        ...
        if (ImGui.Begin("Lab13", ImGuiWindowFlags.HorizontalScrollbar))
        {
            ...
            {
                ParseTextureCase("texture001", "Empty file, unexpected EOF");
                ParseTextureCase("texture002", "Missing texture data, unexpected EOF");
                ParseTextureCase("texture003", "Incorrect number of dimensions");
                ParseTextureCase("texture004", "Failed to parse color component");
                ParseTextureCase("texture005", "Dimensions must be greater than 0");
                ParseTextureCase("texture006", "Dimensions must be greater than 0");
                ParseTextureCase("texture007", "Incorrect number of color components");
                ParseTextureCase("texture008", "Failed to parse dimensions");
                ParseTextureCase("texture009", "Excessive texture data");
                ParseTextureCase("texture010", "Color component out of range");
            }
    ...
    private void ParseTextureCase(string texture, string message)
    {
        var path = $"Resources/{texture}.tex";
        if (ImGui.Button($"{path} - {message}"))
        {
            try
            {
