...
GameSolver::GameSolver(uint _max_value, uint _column_count, uint _row_count, const std::vector<uint>& _column_sum, const std::vector<uint>& _row_sum):
	m_max_value(_max_value), m_column_count(_column_count), m_row_count(_row_count), m_column_sum(_column_sum), m_row_sum(_row_sum), m_sum(0), m_temp_sum(0)
{
...
void GameSolver::RecursiveSolve(uint index, std::vector<Board>* solutions)
{
	...
	{
		if (m_sum - m_temp_sum > m_max_value * (size - index))
			break;

		uint index_row = index / m_column_count;
		uint index_col = index % m_column_count;
		uint max_value = std::min(m_max_value, std::min(m_column_sum[index_col] - m_temp_column_sum[index_col], m_row_sum[index_row] - m_temp_row_sum[index_row]));

		for (uint index_value = 1; index_value <= max_value; index_value++)
		{
			m_temp_column_sum[index_col] += index_value;
			m_temp_row_sum[index_row] += index_value;
			m_temp_sum += index_value;
			m_board[index_row][index_col] = index_value;
